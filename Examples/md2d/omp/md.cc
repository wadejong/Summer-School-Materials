...
#pragma omp single
	{
	  ...
	  npair = (npair-1)/nthread + 1; // Target number of pairs per thread

	  for (int i=0; i<(nthread-1); i++) {
	      while(thrneigh[i].size() < npair) { // steal pairs from next thread
		  if (thrneigh[i+1].size() == 0) break;
		  thrneigh[i].push_back(thrneigh[i+1].back());
		  thrneigh[i+1].pop_back();
	      }
	      while(thrneigh[i].size() > npair) { // donate pairs to next thread
...
coordT forces(const thrneighT& thrneigh, const coordT& coords, double& total_virial, double& total_pe) {
    double start = omp_get_wtime();
    coordT total_f(natom,xyT(0.0,0.0));
...
#pragma omp parallel default(none) shared(total_f, thrneigh, coords, total_virial, total_pe)
    {
...
#pragma omp critical 
	{
	    ...
	    for (int i=0; i<natom; i++) {
	        total_f[i].first += f[i].first;
	        total_f[i].second += f[i].second;
	    }
	}
    }
