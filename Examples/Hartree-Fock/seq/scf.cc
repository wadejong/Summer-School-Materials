    ...
    for(auto s2=0; s2!=shells.size(); ++s2) {
      ...
      for(auto s3=0; s3!=shells.size(); ++s3) {
        ...
        for(auto s4=0; s4!=shells.size(); ++s4) {
          ...
          if (buf_1234 != nullptr) {  // if not all integrals screened out, compute the Coulomb contribution
          ...
          if (buf_1324 != nullptr) {  // if not all integrals screened out, compute the exchange contribution
